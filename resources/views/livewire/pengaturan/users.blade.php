            ...
            <div class="card-body">
                <div class="table-responsive">
                    <div class="row justify-content-between mb-2">
                        ...
                        <div class="col-4">
                            <div class="d-inline" wire:ignore>
                                <select class="form-select" wire:model="role_id" wire:change="filterAkses"
                                    data-pharaonic="select2" data-component-id="{{ $this->id }}"
                                    data-search-off="true" data-placeholder="== Filter Hak Akses ==">
                            ...
                            @if ($data_user->total())
                                @foreach ($data_user as $user)
                                    <tr>
                                        ...
                                        <td>{{ $user->email }}</td>
                                        <td>
                                            {{ $user->allPermissions()->unique()->implode('display_name', ', ') }}
                                        </td>
                                        <td>{{ $user->last_login_at }}</td>
                                        <td>{!! \Illuminate\Support\Facades\Hash::check($user->default_password, $user->password)
                                            ? $user->default_password
                                            : '<span class="btn btn-sm btn-success"> Custom </span>' !!}</td>
                                        <td class="text-center">
                                            <button class="btn btn-success btn-sm"
                                                wire:click="view('{{ $user->user_id }}')"><i class="fas fa-eye"></i>
                                                Detil</button>
                                        </td>
                                    </tr>
                                @endforeach
                    ...
                    <div class="row justify-content-between mt-2">
                        <div class="col-4">
                            @if ($data_user->count())
                                <p>Menampilkan {{ $data_user->firstItem() }} sampai
                                    {{ $data_user->firstItem() + $data_user->count() - 1 }} dari
                                    {{ $data_user->total() }} data</p>
                            @endif
                        </div>
                        <div class="col-4">
