...
class Users extends Component
{
    ...
    public function render()
    {
        ...
        $where = function ($query) {
            $query->whereHasRole(['guru', 'siswa', 'tu']);
            ...
            foreach ($data as $d) {
                ...
                if ($find_rombel) {
                    if (!$user->hasRole($WalasRole, session('semester_id'))) {
                        $user->addRole($WalasRole, session('semester_id'));
                    }
                } else {
                    if ($user->hasRole($WalasRole, session('semester_id'))) {
                ...
                if ($find_mapel_p5) {
                    if (!$user->hasRole($p5Role, session('semester_id'))) {
                        $user->addRole($p5Role, session('semester_id'));
                    }
                } else {
                    if ($user->hasRole($p5Role, session('semester_id'))) {
                ...
                if ($find_ekskul) {
                    if (!$user->hasRole($PembinaRole, session('semester_id'))) {
                        $user->addRole($PembinaRole, session('semester_id'));
                    }
                } else {
                    if ($user->hasRole($PembinaRole, session('semester_id'))) {
    ...
    public function update()
    {
        foreach ($this->akses as $akses) {
            if (!$this->pengguna->hasRole($akses, session('semester_id'))) {
                $this->pengguna->addRole($akses, session('semester_id'));
            }
        }
